...
final class AlgorithmTelemetryGitHubClient {
    ...
    /// Initializes with a single README so the branch isn't empty (GitHub requires content).
    func initializeBranchIfMissing(repo: String, branch: String, token: String) async throws {
        ...
        // 1) Create blob with README content
        let readmeContent = """
        # Trio Telemetry
        ...
        Files are organized as `telemetry/YYYY-MM/DD/<file>.{jsonl,json}`.

        This branch is intentionally an orphan (no shared history with main) so it
        doesn't clutter the code branch.
        """
        let blobSHA = try await createBlob(repo: repo, content: readmeContent, token: token)

        // 2) Create tree containing only the README
        let treeSHA = try await createTree(
            repo: repo,
            entries: [TreeEntry(path: "README.md", mode: "100644", type: "blob", sha: blobSHA)],
            baseTree: nil,
            token: token
        )

        // 3) Create root commit with NO parents (orphan)
