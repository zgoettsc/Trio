...
/// Markdown documents that ship in the initial orphan-branch commit. Single source of
/// truth for the README + the analysis-methods runbook so we can edit one place and
/// the next branch init picks up the new content.
///
/// To update the doc after the branch already exists: edit here, push a new commit to
/// the source branch, then either re-run `Initialize Telemetry Branch` in the app
/// (a no-op for the ref but writes the README content)... or just edit the file
/// directly in the GitHub web UI. The version here is what NEW users get.
enum AlgorithmTelemetryDocs {
    static let readme: String = """
    # Trio Telemetry
    ...
    static let analysisMethods: String = """
